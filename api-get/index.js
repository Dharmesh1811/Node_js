...
app.post('/add', upload.none(), async (req, res) => {
    try {
        const { id, name, post, salary } = req.body;
        console.log(req.body)
        if ( !id || !name || !post || !salary ) {
            return res.status(400).send({ error: 'Name and Post are required fields' });
        }

        const obj = { id, name, post, salary };
        console.log('Form Data:', obj);

        let db = await main()
        let collection = db.collection('salary');
...
app.put('/update', upload.none(), async (req, res) => {
    try {
        ...
        const updateDoc = { $set: { id, name, post, salary } };
...
app.delete('/delete', upload.none(), async (req, res) => {
    try {
        ...
        const filter = { _id: new ObjectId(id) };
        const Delete = { $set: { id, name, post, salary } };
